  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
        <Link href="/portal/invoices" className="btn btn-secondary" style={{ fontSize: '0.85rem' }}>← Back</Link>
      ...
      <div id="invoice-print" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '2.5rem', maxWidth: 800, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginBottom: '2rem' }}>
          ...
          <div style={{ textAlign: 'end' }}>
            ...
            <p style={{ fontSize: '0.875rem', marginTop: '0.25rem', fontWeight: 600, color: invoice.status === 'paid' ? 'var(--success)' : 'var(--warning)' }}>{invoice.status?.toUpperCase()}</p>
          </div>
        </div>
      ...
      <style>{`@media print { nav, .btn, a { display: none !important; } #invoice-print { border: none !important; } }`}</style>
    </div>
  )
}
